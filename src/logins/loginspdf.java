...
/**
 *Clase para crear pdfs con logins fallidos o exitosos
 * @author Luis David Vidales Ramírez, 183060, 183060@upslp.edu.mx
 */
public class loginspdf {
    private final loginsLBD obj;
    private int i;
    /**
     * Constructor
     * @param obj Objeto de tipo lista de objetos de tipo login
     */
    public loginspdf(loginsLBD obj){
        this.obj=obj;
        this.i=0;
    }
    /**
     * Metodo que crea el pdf de logins
     * @param nombre Nombre del pdf
     * @param exito Si son logins exitosos o fallidos
     */
    public void pdfLogins(String nombre, int exito){
        ...
        try{
            ...
            g.setFont(font1);
            g.setColor(Color.black);
            g.drawString("Consulta de Logueos", 130, 60);
            ...
            g.setFont(fontfinal);
            g.setColor(Color.BLACK);
            g.drawString("Almacen Polilinea | Consulta de Logueos",150,750);
            ...
            do{
               ...
               /* font8= new Font("Tahoma",Font.BOLD+Font.ITALIC,35);
                g.setFont(font8);
                ...
                g.setColor(Color.black);
                if(exito==0){
                       g.drawString("Logueos fallidos al sistema", 100, 40);
                }else{
                       g.drawString("Logueos exitosos al sistema", 100, 40);
                }
                ...
                int x=100;
                int cont=0;
                while(i<obj.getLista().size() && cont<5){
                   g.drawString("Username: "+obj.getLista().get(i).get(0).toString(),x,y);
                   y+=20;
                   g.drawString("Password: "+obj.getLista().get(i).get(1).toString(),x,y);
                   y+=20;
                   g.drawString("Fecha: "+obj.getLista().get(i).get(2).toString(),x,y);
                   y+=20;
                     g.drawString("Hora: "+obj.getLista().get(i).get(3).toString(),x,y);
                   y+=50;
                   i++;
                   cont++;
                }
